...
export function RecipeHeaderInfo({ recipe }: { recipe: Recipe }) {
  return (
    <div className="space-y-4">
      {recipe.cuisine && (
        <Badge variant="secondary" className="bg-gray-100 text-gray-600 hover:bg-gray-100 rounded-full px-3 py-1 text-sm">
          {recipe.cuisine}
        </Badge>
      )}
      ...
      <div className="flex flex-wrap items-center gap-3 text-gray-500 pt-2">
        <div className="flex items-center gap-2 bg-white border border-gray-200 px-4 py-2 rounded-full">
          ...
          <span className="font-medium text-gray-900">{recipe.cooking_time_in_minutes} min</span>
        </div>
        <div className="flex items-center gap-2 bg-white border border-gray-200 px-4 py-2 rounded-full">
