...
export function useHomeRecipes() {
  ...
  // Infinite query for browsing recipes (not searching)
  const {
    data: infiniteData,
    ...
    getNextPageParam: (lastPage, _allPages, lastPageParam) => {
      // If last page returned fewer items than PAGE_SIZE, no more pages
  ...
  // Search uses regular query (no pagination on search endpoint)
  const {
    data: searchResults,
